...
;(function (global, factory) {
	...
	function addEdge(startNode, endNode, direct, attrs) {	//	direct: none, forward, back, both
		...
		var d = direct;
		startNode.connectedNodes[endNode.uuid] = (d == "forward" ? "-" : (d == "back" ? "<" : (d == "both" ? "<" : "-")));
		endNode.connectedNodes[startNode.uuid] = (d == "forward" ? "<" : (d == "back" ? "-" : (d == "both" ? "<" : "-")));
	...
	function Table(attrs) {
		...
		var table = {
			...
			attrs: function() {
				var label = [];
				label.push(`<<TABLE${toTagAttrs(attrs)}>`);
	...
	function Td(table, index, name, attrs) {
		...
		var td = {
			...
			attrs: function() {
				attrs.label = `<TR><TD PORT="${index}"${toTagAttrs(attrs)}>${this.name}</TD></TR>`;
				return attrs;
			},
		};
...
`);
	...
	function highlight(config) {
		return function(element, graphviz) {
			...
			//style.type = 'text/css';
			//style.sheet.insertRule('svg .myclass { fill: red; }');
				...
				return function(event, d) {
					...
					edges.select(function(o) {
						if (this.initHighlight == null) {
							...
							this.polygon = this.querySelector("polygon");	// arrow
							if (this.polygon) {
								this.polygon_stroke = this.polygon.getAttribute("stroke");
								this.polygon_fill = this.polygon.getAttribute("fill");
							}
						...
						if (value == 1) {
							...
							if (this.polygon) {
								this.polygon.style.stroke = config.color;
								this.polygon.style.fill = config.color;
							}
						}
			...
			function mouseOut(event, d) {
				...
				edges.select(function(o) {
					...
					if (this.polygon) {
						this.polygon.style.stroke = this.polygon_stroke;
						this.polygon.style.fill = this.polygon_fill;
					}
