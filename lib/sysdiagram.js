...
;(function (global, factory) {
	...
	function Td(table, index, name, attrs) {
		...
		td._$ = td.inout;		// alias (>>) ->
		td.$_$ = td.both;		// alias (<< >>) <->
		td.left = td.outin;		// alias (<<)
		td.right = td.inout;	// alias (>>)
		td.e = td.edge;			// alias (edge)
	...
	function generateDotEdge(lines, edges) {
		for (var key in edges){
			...
			if (attrs.sport != null || attrs.eport != null) {
				...
				delete attrs.sport;
				delete attrs.eport;
				lines.push(`	${edge.startnode.uuid + sport} -> ${edge.endnode.uuid + eport} ${toAttrs(attrs)}`);
			} else {
				lines.push(`	${edge.startnode.uuid} -> ${edge.endnode.uuid} ${toAttrs(attrs)}`);
			}
		}
	}
